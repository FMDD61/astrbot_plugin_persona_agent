...
MEMBERS = {
    "members": [
        {"uin": "1", "alias": "成员丙", "other_names": ["成员丙的小名"], "closeness": "close"},
        {"uin": "2", "alias": "成员乙", "other_names": ["成员乙的小名", "成员乙的昵称"], "closeness": "close"},
        {"uin": "3", "alias": "路人", "other_names": [], "closeness": "new"},
        # 歧义：两个成员共享同一个 other_name
        {"uin": "4", "alias": "成员壬", "other_names": ["共用别名"], "closeness": "close"},
        {"uin": "5", "alias": "成员癸", "other_names": ["共用别名"], "closeness": "close"},
    ]
}
...
class TestStrictNameResolution(unittest.TestCase):
    ...
    def test_alias_exact(self):
        self.assertEqual(self.sp.resolve_member_name("成员丙"), ("1", "alias"))

    def test_other_name_exact(self):
        self.assertEqual(self.sp.resolve_member_name("成员乙的小名"), ("2", "other_name"))
    ...
    def test_ambiguous_is_rejected_not_guessed(self):
        ...
        实测真实数据里 `成员壬` 同时属于 `成员壬` 与 `成员癸` ——
        旧 `resolve_uin_from_name` 会返回首个匹配 → **戳错人**（对外可见的社交事故）。
        """
        uin, why = self.sp.resolve_member_name("共用别名")
        self.assertIsNone(uin)
        self.assertIn("ambiguous_other_name", why)
        # 对照：旧解析器确实会猜（证明这个新方法的必要性）
        self.assertEqual(self.sp.resolve_uin_from_name("共用别名"), "4")
    ...
    def test_case_insensitive(self):
        assert self.sp.resolve_member_name("成员丙")[0] == "1"
