...
class PokeDecideTests(unittest.TestCase):
    ...
    def _decide(self, poker="234567", known=True, text="", now=None):
        return self.svc.decide(now_utc=now, poker=poker, group_id="123456789",
    ...
    def test_cooldown_blocks_then_expires(self) -> None:
        ...
        self.assertTrue(ok)
        self.svc.record(now_utc=t0, poker="234567", group_id="123456789", responded=True, reason="ok")
...
class PokeLogTests(unittest.TestCase):
    def setUp(self) -> None:
        self.tmp = tempfile.TemporaryDirectory()
        self.dir = Path(self.tmp.name)
        self.svc = PokeService(str(self.dir), bot_qq="123456")

    def tearDown(self) -> None:
        self.tmp.cleanup()

    def test_log_appended_jsonl(self) -> None:
        self.svc.record(now_utc=4_000_000.0, poker="234567", group_id="123456789",
                        responded=True, reason="ok")
        lines = (self.dir / "poke_log.jsonl").read_text("utf-8").splitlines()
        self.assertEqual(len(lines), 1)
        entry = json.loads(lines[0])
        self.assertEqual(entry["poker"], "234567")
    ...
    def test_keywords_hot_reload_from_file(self) -> None:
        ...
        svc2 = PokeService(str(self.dir), bot_qq="123456")
