...
class TestStopwords(unittest.TestCase):
    ...
    def test_real_topics_survive(self):
        """群内真实梗必须保留 —— 宁可漏删不可误删。"""
        for w in ("老婆", "戒色", "签到", "晚安", "早睡", "焦糖", "纸鸢",
                  "提科", "哈基蝶", "报错", "日志", "名字", "Q版", "二次元"):
...
def _stub_keywords(text: str) -> list[str]:
    ...
    测试文本用 `"提科 真不错"` 形式给出确定的多词；`"提科"` 单串也当一词。
    """
    t = str(text or "").strip()
    if not t:
        return []
...
class TestImageDescNotIngested(unittest.TestCase):
    ...
    def test_failed_image_placeholder_produces_no_topics(self):
        self.store.ingest(MemoryEvent(
            speaker_alias="焦糖", text="（配图：无法识别）", group_id="g",
        ))
...
class TestTopicOccurrenceThreshold(unittest.TestCase):
    ...
    def test_second_occurrence_creates_edge(self):
        ...
        for _ in range(2):
            s.ingest(MemoryEvent(speaker_alias="甲", text="提科 真不错", group_id="g"))
        edges = self._edges()
        self.assertIn("提科", edges)
        self.assertEqual(edges["提科"], 1)   # 只在第二次建了一条

    def test_third_occurrence_keeps_adding(self):
        s = self._store(2)
        for _ in range(3):
            s.ingest(MemoryEvent(speaker_alias="甲", text="提科 真不错", group_id="g"))
        self.assertEqual(self._edges()["提科"], 2)   # 第2、3次各一条

    def test_threshold_one_disables_gate(self):
        """min_occurrences=1 → 退回旧行为（每条都建边），用作回滚开关。"""
        s = self._store(1)
        s.ingest(MemoryEvent(speaker_alias="甲", text="提科 真不错", group_id="g"))
        self.assertIn("提科", self._edges())
