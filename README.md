...
**v0.4.0（2026-08-24）** — 已部署运行（`test_mode=1`，仅测试群 `123456788` 生效）。含：v3 按日会话+02:00轮换、睡眠窗 02–07、每日日记（复用会话前缀缓存）、识图（flash-vision-exp）、情绪引擎 v1、示例注入（规则A/B）、离线 A/B 通道。未启用（骨架或开关=0）：active_interjection / topic_bank / poke / dream。
