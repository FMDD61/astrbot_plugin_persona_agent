...
[system]  固定示例块（G14）                      ← 恒定（S2 从末尾移到这里）
[user]    …session 全量历史…                     ← 只追加（**不含当前这条**）
[system]  【现在要回应的】@ 提示 + 发话人
          + 正文 + ［图片］/［表情］（直投式）
          + 【当下】时间 + 心情                  ← 逐轮变，只破坏尾部
[system]  KG 尾注                                ← 每轮变
```

> **恒定块必须在增长段之前**：示例块原先拼在 session 之后，而 session 每轮变长
> （实测一天 1482 条）→ 排在增长段之后的恒定内容永远落在缓存失效区。
> **本条不入上下文前缀**：main 只挂起，pipeline 在上下文/引用快照之后才落盘，
