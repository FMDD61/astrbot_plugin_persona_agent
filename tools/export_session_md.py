...
## 用法
    ...
    python3 -m tools.export_session_md \\
        --session /path/session_100000001_2026-09-16.json --out review.md
    ...
    python3 -m tools.export_session_md --session ... --out out.md \\
        --extra-from-trace /path/logs/100000001/trace_log.jsonl
"""
...
def export_markdown(session_path: Path, out_path: Path, *,
    ...
    parts: list[str] = [
        ...
        f"- 思维链：{'有' if any(m.get('_reasoning') for m in rendered) else '未留存'}",
        "",
        "> 说明：**思维链不进 LLM 上下文**，但列在这里供人工排查。",
        "> `_mid`/`_uin` 等内部元数据同样不进 LLM 请求。",
        "> ⚠️ 显示「未留存」时**不代表模型没思考**：2026-09-17 之前写入的会话受",
        "> B-029 影响（网关字段名是 `reasoning` 而代码读的是 `reasoning_content`）",
        "> 从未留存；此后新写入的条目应能看到思维链。",
        "",
        "---",
        "",
    ]
