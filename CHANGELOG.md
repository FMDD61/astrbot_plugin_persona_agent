...
- **自检误报「§3 是旧的」（时区口径）**：那句判据比的是 `generated_at` 的 **UTC 日期**与
  **今天的 UTC 日期**，而轮转在 02:05 CST = **前一天的 18:05 UTC** → 每天 00:00-08:00 CST
  之间必然误报。改成**显式 +8**（与 `digest_fresh_today` 同口径），告警文案也带上比对基准。
  ⚠️ 与我刚修的 `digest_fresh_today` 是**同一类错**，那处改对了、这处漏了。
  🔁 **同一处我错了两次**：第一版只把「今天」换成 CST，左边仍拿 `_gen[:10]`（UTC 裸串）当日期 →
  部署后 13:17:34 实测**照旧误报**。**必须两边都换算**；闸也收紧为「必须出现 `astimezone(_cst)`」
  且禁止 `_gen[:10] < _today_cst` 这种比法（去掉转换的变异现在会红）。
- **P3（审查第 2 轮）：判据改用「会话日」**。轮转把 00:00-02:00 归前一天，而判据原先比 CST
  **日历日** → 两个口径不一致的时段（每天 00:00-02:05）判据恒"不新鲜"，又变成**每条消息重建一次**
  （只是日志已 DEBUG，更隐蔽）。现在 `digest_fresh_today(..., day=)` 接收 `session_mgr.day_key()`，
  由 main 传下来；不给 `day`（单测/离线）时退回 CST 日历日。
- 测试：+3 例（会话日优先 / 自检必须 CST / 判据必须吃会话日），其中前两条**变异自证有牙**
  （改回 UTC 或忽略会话日 → 用例红）。898 OK。
- 📌 观察记录：`runtime.log` **每行写两遍且有 NUL 字节**（start 脚本的 `tee -a` 与 AstrBot 自身
  的文件 handler 同时写同一文件）→ 任何「按时间窗口 grep」的取证都可能把几天前的同刻日志算进来
  （我这次被连骗两次）。取证请改用**行号锚定**（先定位最后一次 `initializing` 的行号，再数其后）。


### Fixed (2026-09-22 续 · 独立审查第 2 轮：P2 日记"日"错位)
- **退避重试跨过 02:00 会把日记写到错误的「日」**：`_generate_diary` 原先**自己现算**
  `day_key(now-86400)`，而重试/补写可能跨过 02:00 边界 → 把「09-20 的归档」写成 day=09-21 的记录；
  后果是**目标日永久缺失**，且这条错位记录会**顶掉 02:05 真正的 09-21 日记**
  （只留一条误导性的"幂等拦截…多实例"）。三条件同时才触发（会话陈旧 ≥1 天 + 01:58–02:00 触发轮转 + 首发失败），
  窄但非零，且后果是**记忆内容错误**。修法：`_generate_diary(..., day="")`，
  `record["day"] = day or day_key(...)`；**轮转 / 重试 / 补写三个调用点都显式传日**。
  ⚠️ 该错位**不是本轮引入**：审查在同探针下复现 `343fb4a` 也有（内部 15 秒 provider 重试跨边界），
  本轮把窗口从 15 秒扩到 2/10 分钟才让它变得容易撞上。
- 补闸："不许在写侧现算日"、"三个调用点都必须传日"、M13 多实例前置检查、M16 重试成功后重算 §3、
  M15 默认参考时刻必须是 CST。**五条变异自证有牙**（其中 M13 第一版没牙 —— 同函数里还有第二处同名
  检查，收紧成"前置检查必须早于第一次生成"后才红）。


### Fixed (2026-09-22 续 · 独立审查第 1 轮：P1 + P2 三条)
- **P1 `_has_diary_for` 读了旧路径（护栏结构性失效）**：读 `<data_dir>/daily_diary.jsonl`
  —— 那是 B-024 的旧路径（内容停在 08-29），日记自 `e566116` 起写在 `logs/<gid>/daily_diary.jsonl`。
  判据对近期日期**恒为「缺失」** → 每次启动白烧一次**全量归档日**的日记 LLM 调用（数据不脏：
  文件级幂等挡住了，但钱白花且留一条误导性 WARNING）。改用 `summary.read_diaries`（双路径 + 同日去重）。
- **P2 时机错位：补写只在进程启动时触发，故障却在 02:05 轮转**。复算 09-22：01:51 启动时
  `day_key(now-86400)`=09-20 → 跳过；02:05 失败 → 09-21 缺失 → **当天不重启就一直缺**。
  新增 `_retry_diary_later()`：轮转里没产出就在本进程内**退避重试**（2 分钟 / 10 分钟），
  仍失败落 WARNING；文案也改成与实现一致（不再说"下次启动兜底"）。
- **P2 时区：判据改用显式 +8（与轮转同口径）**。生产 `TZ=Asia/Shanghai` 下旧写法成立，
  但探针实测 `TZ=UTC` 时 ~18h/天恒不新鲜 → 退化成"每条消息重读四层摘要"，而日志已降 DEBUG
  = **完全静默**（B-056 换马甲）。
- **P2 静默路径**：`summary.digest_fresh_today` 的异常、`_has_diary_for` 的异常都改成
  **按陈旧/缺失处理 + WARNING 留痕**（前者每进程一次去重）。
  ⚠️ 修这条时又踩同类坑：`services/summary.py` **原本没有 `logger`** —— 我新加的告警会在
  `except` 里抛 `NameError`。已补 `logging.getLogger(__name__)`（本文件开头记的就是这类事故）。
- 📌 **§3 预期管理**：补写成功 ≠ §3 立刻丰富。09-22 的 `layers={}` + `skipped_no_digest=3` 是
  **C31 之前的旧记录没有 `digest`** 导致的（用户定案「无 digest 视作空」）→ 补写后
  **只有带围栏的新日记才会多出一行**，旧日记不会凭空变出记忆。
...
> **教训：交付「自动定时」脚本时，要把它依赖的前提（这里=代码在远端）也验一遍。**

- 部署方式：`~/bin/deploy_persona_plugin --force-restart`（`tools/deploy/`；**只重启 AstrBot**，容器/QQ 未动）。
...
- **离线兜底**：定时任务里带 `--force-restart`，所以凌晨 `git fetch` 因网络/凭据失败时，
  仍会用**盘上现有代码**跑测试并重启（否则凌晨那一跑会静默地什么都没做）；不带该参数则照旧中止。
- 测试：插件 Python 套件不受影响（**778 全绿**）；另加 shell 的 **37 断言 / 11 场景**自测台。
