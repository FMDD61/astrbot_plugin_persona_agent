...
class StyleProfile:
    ...
    # ⚠️ 这里的数字是**合成占位**（= 已知占位 epoch `1700000000` + 占位号 `123456789`）：
    # 2026-09-23 脱敏第二轮把实测值换掉了 —— 实测的纳秒时间戳切出 10 位前缀后，
    # 与真实 QQ 号**无法从形态上区分**（第一轮就是因为这个把真号当成"假时间戳"漏了），
    # 所以例子值也必须是不含真实数字的合成值。技术含义（两次写入 ns 相同）不变。
