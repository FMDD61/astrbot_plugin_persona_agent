...
| EmotionProvider | services/emotion.py | ScoreEmotionProvider **C24 代码计算，无 LLM 调用**：score 起 1.0，Gate 每判一次 `blocked` −0.1、随时间 +0.1/分恢复，硬闸乘子 = 0.6 + 0.4×score；旋钮全在 `emotion.*`（`enabled`/`initial_score`/`blocked_penalty`/`recovery_per_min`/`min_score`/`max_score`/`recovery_log_step`）；每次扣分落 INFO（测量仪器）；`enabled=0` 恒中性、零日志 |
...
- **LLM 参数矩阵 (A7④，2026-09-13 修订)**: 采样参数（temperature/reasoning_effort）不进 prompt；RP/Diary/Summary 与 Gate 结构化任务**统一 `low`**（**Emotion 自 C24 起不再调 LLM**，没有采样参数） + 温度分档（RP 按场景，结构化 0.2）；**reasoning 映射语义：off/未知 → None = 不发送该参数**（`services/llm_params.py`；网关对 none/off/minimal/disabled/false 一律 400，dsh 自身 `reasoningEfforts:{False:None}` 同义）；`llm.max_tokens` 默认 **512**（需容纳 200–800 思考 token，256 会截空 content；此前是**死配置**，2026-09-13 才接线）；provider 同源（统一走 `_resolve_provider_id()`：RP/Gate/Vision/Dream/关系提案/Summary，启动即预热；**Emotion 自 C24 起不调 LLM、不需要 provider**）。
