...
- Sender identity: `sender.uin` is a numeric string (e.g. `"337934842"`). Comparing to int silently fails.
- `senders[]` contains bookkeeping entries (e.g. `uid:"881438753"` is NOT a real user). Don't iterate `senders[]` to find people.
